...
def ernie_128k(speech_string, group_name):
    ...
    """
    prompt_pre = r"""
    # 目的：请提供群聊的聊天对话文本，并总结聊天内容最核心的3-5个话题。请严格按照以下要求和格式操作。
    
    # 前提：每个话题总结要简洁，用5-10个字总结即可，请再三确认字数在5-10个字。（如果有敏感内容，你直接跳过敏感内容即可）
    
    # 格式要求，请按照以下严格格式总结话题，格式举例：
    ##       举例一： 
           1. 健康生活。
           2. 减脂增肌。
           3. 打卡与分享。
    ...
    def re_clean_text(text):
        ...
        # 对match_str进行正则处理，添加emoji
        match_str = re.sub(r'(\d+\.)', r'💬 \1', match_str)  # \1代表第一个捕获组，即数字和点。

        # print(match_str)  # 将匹配项以逗号分隔的字符串)
        return match_str

    match_str = re_clean_text(str(_result))
    match_str = '无' if match_str == "" else match_str  # 若是无内容则输出提示

    ctn_summary = f'🌟 今日话题总结:\n\n{match_str}'  # \n{group_name}\n
    print(ctn_summary)
...
def fetch_content():
    ...
    for group_name in group_names:
        ...
        if len(df2) == 0:
            ...
            print(f'聊天内容：{talk_content}')
            ernie_128k(talk_content, group_name)


## ods2dw
df = ods2dw_wechat()  # 将ods转为dw，
fetch_content()  # 2. 话题总结 （因要放在后面，先运行）
df_analyze(df)  # 1. 发言排行

# x 5-10个字的限制会导致过于模糊， ：测试为10-15个会有意料之外的问题。暂不处理
# 3个话题可能少了，需要将prompt设置为智能判断话题数。
